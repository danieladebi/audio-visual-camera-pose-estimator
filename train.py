...
from reloc3r.reloc3r_relpose import Reloc3rRelpose, PolicyClassificationModel, AudioDOAModel, DOACameraPoseModel, SLfMNet
...
def main(args):
    ...
    # TODO: implement gradient blending with different models
    # Also TODO: make audio only mode and joint model, and use gradient blending to combine all of them
    ...
    # TODO: add gradient blending
    # train_losses = {}
    # val_losses = {}

    # g_blend_freq = 5
    # weights = [0.85, 0.05, 0.1]
    # train_criterion.reset_weights(weights)
    # test_criterion.reset_weights(weights)
    for epoch in range(args.start_epoch, args.epochs+1):
        ...
        # if epoch % g_blend_freq == 0 and epoch > 0:
        #     # m_names = ["visual", "audio", "joint"]
        #     weights_sum = 0
        #     for m_i in range(3):
        #         O_Nn = val_losses[epoch][m_i] - train_losses[epoch][m_i]
        #         if epoch - g_blend_freq not in train_losses:
        #             train_losses[epoch - g_blend_freq] = train_losses[1]
        #         if epoch - g_blend_freq not in val_losses:
        #             val_losses[epoch - g_blend_freq] = val_losses[1]
        #         O_N = val_losses[epoch - g_blend_freq][m_i] - train_losses[epoch - g_blend_freq][m_i]
                
        #         G_Nn = abs(val_losses[epoch][m_i] - val_losses[epoch - g_blend_freq][m_i])
        #         O_i = abs(O_Nn - O_N)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    # Calculate mean for each value in tuple
    # if losses and isinstance(losses[0], tuple):
    #     losses_mean = [0,0,0]
    #     for i in range(len(losses_mean)):
    #         for j in range(len(losses)):
    #             losses_mean[i] += losses[j][i].item() / len(losses)
    # else:
    #     losses_mean = torch.mean(losses)
    
    # del losses
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()} #, losses_mean
...
def test_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
   ...
   # losses = []
    ...
    # Calculate mean for each value in tuple
    # if losses and isinstance(losses[0], tuple):
    #   #  losses_mean = tuple(torch.mean([loss[i] for loss in losses]) for i in range(len(losses[0])))
    #     losses_mean = [0,0,0]
    #     for i in range(len(losses_mean)):
    #         for j in range(len(losses)):
    #             losses_mean[i] += losses[j][i].item() / len(losses)
